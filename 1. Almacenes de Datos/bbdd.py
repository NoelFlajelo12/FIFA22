...
df_jugador = df_jugadores[['sofifa_id','short_name','long_name','age','dob','height_cm','weight_kg','nationality','club','overall','player_positions']].copy()
df_jugador.to_sql('jugador', engine, chunksize=500000,
          method='multi', index=False, if_exists='replace')

df_club = df_jugadores[['sofifa_id','club','team_position','team_jersey_number' ]].copy()
df_club.to_sql('club', engine, chunksize=500000,
            method='multi', index=False, if_exists='replace')
            
df_nacionalidad = df_jugadores[['sofifa_id','nationality','nation_position','nation_jersey_number' ]].copy()
df_nacionalidad.to_sql('nacionalidad', engine, chunksize=500000,
            method='multi', index=False, if_exists='replace')

df_habilidad = df_jugadores[['sofifa_id','overall','potential','international_reputation','skill_moves','weak_foot', 'work_rate']].copy()
df_habilidad.to_sql('habilidad', engine, chunksize=500000,
            method='multi', index=False, if_exists='replace')

df_habilidad_jugador = df_jugadores[['sofifa_id','player_positions','pace','shooting','passing','dribbling','defending','physic']].copy()
df_habilidad_jugador.to_sql('habilidad_jugador', engine, chunksize=500000,
            method='multi', index=False, if_exists='replace')

df_habilidad_portero = df_jugadores[['sofifa_id','player_positions','gk_diving','gk_handling','gk_kicking','gk_reflexes','gk_speed','gk_positioning']].copy()
df_habilidad_portero.to_sql('habilidad_portero', engine, chunksize=500000,
            method='multi', index=False, if_exists='replace')

df_fisico = df_jugadores[['sofifa_id','height_cm','weight_kg','attacking_heading_accuracy','movement_agility','movement_reactions','power_jumping','power_strength','power_stamina']].copy()
